...
float estimateRelativePosition(vec2 dir, int maxSearch, float shadow) {
	...
	return (max(T, 2 * shadow - 1) * abs(max(T * dir.x, T * dir.y)))/edgeLength;
...
float revectorizeShadow(vec2 r, float shadow)
{
	...
	if((r.x * r.y == 2 * shadow) ||
		((abs(r.x) * abs(r.y) > 0) && ((1.0 - shadow) + (2 * shadow - 1) * (abs(r.x) + abs(r.y)) < 0.5))) {
		return 0.0;
	} else return 1.0;
...
float computeShadow(vec4 normalizedLightCoord)
{
...
{	
	...
	if(shadow == 1.0) shadow = computeShadow(normalizedLightCoord);
