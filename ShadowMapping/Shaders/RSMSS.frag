	...
	if(discontinuity.b == 0.0) {
		...
		if(discontinuity.r > 0.0 && discontinuity.g > 0.0) {
			...
			//If there is no dominant axis
				...
				else if(discontinuity.g == 1.0) return clamp((1.0 - normalizedDiscontinuity.x) + subCoord.y, 0.0, 1.0);
		...
		if(normalizedDiscontinuity.x <= -2.0) return mix(1.0 - subCoord.y, subCoord.y, step(1.0, discontinuity.g));

		//If positive discontinuity only in y-axis
		if(normalizedDiscontinuity.y <= -2.0) return mix(subCoord.x, 1.0 - subCoord.x, step(1.0, discontinuity.r));
	...
	//For exiting discontinuity
		...
		if(discontinuity.r > 0.0 && discontinuity.g > 0.0) {
			...
			//If there is no dominant axis
				...
				//If top discontinuity
				else if(discontinuity.g == 1.0) return clamp((1.0 - subCoord.y) - (1.0 - normalizedDiscontinuity.x), 0.0, 1.0);
		...
		//If positive discontinuity only in x-axis
		if(normalizedDiscontinuity.x <= -2.0) return mix(subCoord.y, 1.0 - subCoord.y, step(1.0, discontinuity.g));

		//If positive discontinuity only in y-axis
		if(normalizedDiscontinuity.y <= -2.0) return mix(1.0 - subCoord.x, subCoord.x, step(1.0, discontinuity.r));
