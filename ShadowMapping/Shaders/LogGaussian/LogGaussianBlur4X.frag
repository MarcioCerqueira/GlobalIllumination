...
vec4 blur(vec2 step, vec2 center)
{
	...
	vec4 sample[4];
	sample[0] = texture2D(image, vec2(center.s - 2 * step.s, center.t));
	sample[1] = texture2D(image, vec2(center.s - 1 * step.s, center.t));
	sample[2] = texture2D(image, vec2(center.s + 1 * step.s, center.t));
	sample[3] = texture2D(image, vec2(center.s + 2 * step.s, center.t));
