...
vec4 blur(vec2 step, vec2 center)
{
	...
	vec4 sample[3];
	sample[0] = texture2D(image, vec2(center.s, center.t - 1.0 * step.t));
	sample[1] = texture2D(image, vec2(center.s, center.t));
	sample[2] = texture2D(image, vec2(center.s, center.t + 1.0 * step.t));
